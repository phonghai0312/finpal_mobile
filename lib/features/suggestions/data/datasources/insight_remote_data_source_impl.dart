...
class InsightRemoteDataSourceImpl implements InsightRemoteDataSource {
  List<Insight> _mockInsights = [
    Insight(
      ...
      message:
          'Chi tiêu cho cà phê tăng 35% so với tháng trước. Bạn đã chi 300.000đ trong tháng này',
      data: {},
      read: false,
    ...
    Insight(
      ...
      message:
          'AI đã quét & phân loại 125 giao dịch từ SMS ngân hàng tháng này',
      data: {},
      read: false,
      createdAt: 1681478400,
      updatedAt: 1681478400,
    ...
    Insight(
      ...
      message:
          'Bạn đã tiết kiệm được 12.500.000đ trong tháng này, tăng 8.5% so với tháng trước',
      data: {},
      read: false,
    ...
    Insight(
      ...
      message:
          'Dựa trên thói quen chi tiêu, bạn nên đặt ngân sách 1.500.000đ cho ăn uống tháng sau',
      data: {},
      read: false,
    ...
    Insight(
      ...
      title: 'Mẹo tiết kiệm',
      message:
          'Thay vì mua cà phê mỗi ngày, bạn có thể tiết kiệm 200.000đ/tháng nếu tự pha tại nhà 3...',
      data: {},
      read: false,
  ...
  Future<List<Insight>> getInsights({
    ...
    final int startIndex =
        ((page ?? 1) - 1) * (pageSize ?? filteredInsights.length);
    final int endIndex = (startIndex + (pageSize ?? filteredInsights.length))
        .clamp(0, filteredInsights.length);
  ...
  Future<Insight> getInsightById(String id) async {
    await Future.delayed(const Duration(milliseconds: 500));
    final insight = _mockInsights.firstWhere(
  ...
  Future<Insight> updateInsight(String id, bool read) async {
    ...
    if (index != -1) {
      _mockInsights[index] = _mockInsights[index].copyWith(
        read: read,
